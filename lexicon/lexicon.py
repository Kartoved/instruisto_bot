...
ABOUT_REPEATING: str = 'Сначала ты учишь слова, используя команду <strong>/learning</strong>. Не надо учить сразу все, начни с пяти слов, капля точит камень. Просто читаешь слово и стараешься его запомнить. Будет вдвойне полезно, если ты будешь использовать мнемотехники (если не знаешь, что это такое, рекомендую изучить по ссылке на кнопке ниже). Далее это слово попадает в список для повторения. Слова из этого списка доступны по команде <strong>/repeating</strong>. Слова там будут попадаться через разные промежутки времени: если ты нажимаешь на "помню", то интервал будет увеличиваться. Если на "не помню" — уменьшаться.Минимальный интервал — один день.\nСтарайся заниматься регулярно. Если будешь учить каждый день по 5 слов, то через 241 день будешь знать все 1204 слова из списка, этого хватит для повседневного общения!'
