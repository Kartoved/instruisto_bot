...
Следующие две команды помогут тебе выучить 1202 самых употребительных слова \
...
🏋️ Старайся заниматься регулярно. Если будешь учить каждый день по 5 слов,\
то через 241 день будешь знать все 1202 слова из списка, \
этого хватит для повседневного общения! 😊\n\n🤗 Благодарности:\n\
...
слова и придётся их учить заново!'
...
— Изучено слов \
<strong>{round(len(list_name)/(1202/100), 1)}%</strong>\
({len(list_name)} из 1202)\n— Сегодня слов для повторения: \
