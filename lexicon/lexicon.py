...
ABOUT_REPEATING: str = (
    '☝️ Сначала ты учишь слова, используя команду \
...
ABOUT_MNEMO: str = (
    '☝️ Мнемотехника, или мнемоника, — это совокупность \
...
RESET_MESSAGE: str = (
    "‼️‼️‼️ Вы уверены, что хотите сбросить прогресс? ‼️‼️‼️\
...
def get_date_of_closest_repetition(explored_words: list) -> str:
    ...
    try:
        ...
        for word in explored_words:
            if (
                datetime.strptime(word["дата повторения"], "%d-%m-%Y")
                < date_of_closest_repetition
            ):
                date_of_closest_repetition = datetime.strptime(
                    word["дата повторения"], "%d-%m-%Y"
                )
        if date_of_closest_repetition.strftime("%d-%m-%Y") <= datetime.now().strftime(
            "%d-%m-%Y"
        ):
            return "Ближайшая дата повторения <strong>сегодня</strong>."
        return f'На сегодня слов для повторения нет.\nБлижайшее повторение слов будет <strong> {date_of_closest_repetition.strftime("%d-%m-%Y")}</strong>'
    except IndexError:
...
def calculate_progress(list_of_words: list, chat_id: int):
    """подсчитать прогресс"""
    with open(f"users_data/{chat_id}/explored_words.json", encoding="utf-8") as f:
        list_of_words = json.load(f)
        memorized_words = [word for word in list_of_words if word["интервал"] <= 1]
        know_good = [word for word in list_of_words if 2 <= word["интервал"] <= 3]
        know_perfect = [word for word in list_of_words if 4 <= word["интервал"] <= 6]
...
def format_learning_message(new_word: dict) -> str:
    ...
    return f"""✍  <em>{new_word['на русском']}</em> → <strong>{new_word['на эсперанто']}</strong>\n\n\
